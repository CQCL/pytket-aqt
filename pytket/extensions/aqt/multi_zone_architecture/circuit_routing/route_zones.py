...
def route_circuit(
    ...
    iteration = 0
    while depth_list:
        current_placement = _new_placement_graph_partition_alg(
            n_qubits, arch, depth_list, current_placement
        )
        depth_list = _get_updated_depth_list(n_qubits, current_placement, depth_list)
        print("DepthLength: ", len(depth_list), "iter: ", iteration)
        if iteration > 400:
            break
        iteration += 1
...
def _initial_placement_graph_partition_alg(
    ...
    max_weight = math.pow(2, min(20, max_considered_depth))
    for i, pairs in enumerate(depth_list):
        if i > max_considered_depth:
            break
        weight = math.ceil(math.exp(-0.5 * i) * max_weight)
        for pair in pairs:
            if pair in edges:
...
def _new_placement_graph_partition_alg(
    ...
    for zone in range(num_zones):
        print("zone ", zone, ": ", initial_placement[zone], new_placement[zone])
    # check for duplicate qubits
    all_qubits = []
